...
export const PersonalInfoSchema = Yup.object({
  ...
  dateOfBirth: Yup.date()
    .nullable()
    .required(i18n.dateOfBirthRequired)
    .max(
...
export const TrackSchema = Yup.object({
  track: Yup.string().oneOf(Object.values(Tracks), i18n.trackRequired).required(i18n.trackRequired),
});
...
export const ToolsSchema = Yup.object().shape({
  frontendTools: Yup.array().when('track', {
    is: Tracks.FRONTEND,
    then: schema => schema.min(1, i18n.toolsRequired).required(),
...
export const ReviewSchema = Yup.object({
  confirm: Yup.boolean().oneOf([true], i18n.confirmRequired).required(),
});
